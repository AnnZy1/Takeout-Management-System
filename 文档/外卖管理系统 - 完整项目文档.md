...
| role_id | bigint | - | 否 | 否 | 否 | 角色 ID | 关联 role.id |
| branch_id | bigint | - | 否 | 否 | 否 | 分店 ID | 关联 branch.id，数据范围控制 |
...
| create_user | bigint | - | 否 | 否 | 否 | 创建人 ID | 关联 employee.id |
| update_user | bigint | - | 否 | 否 | 否 | 最后修改人 ID | 关联 employee.id |
...
| role_id | bigint | - | 否 | 否 | 否 | 角色 ID | 关联 role.id |
| permission_id | bigint | - | 否 | 否 | 否 | 权限 ID | 关联 permission.id |
...
| branch_id | bigint | - | 否 | 否 | 否 | 分店 ID | 关联 branch.id，支持分店独立分类 |
| is_deleted | tinyint | - | 否 | 否 | 否 | 软删除标识 | 1 - 已删 0 - 未删（默认 0） |
| create_time | datetime | - | 否 | 否 | 否 | 创建时间 | 默认 CURRENT_TIMESTAMP |
| update_time | datetime | - | 否 | 否 | 否 | 最后修改时间 | ON UPDATE CURRENT_TIMESTAMP |
| create_user | bigint | - | 否 | 否 | 否 | 创建人 ID | 关联 employee.id |
...
| category_id | bigint | - | 否 | 否 | 否 | 分类 ID | 关联 category.id（type=1） |
...
| status | tinyint | - | 否 | 否 | 否 | 售卖状态 | 1 - 启售 0 - 停售（默认 1） |
| branch_id | bigint | - | 否 | 否 | 否 | 分店 ID | 关联 branch.id，支持分店独立菜品 |
| is_deleted | tinyint | - | 否 | 否 | 否 | 软删除标识 | 1 - 已删 0 - 未删（默认 0） |
| create_time | datetime | - | 否 | 否 | 否 | 创建时间 | 默认 CURRENT_TIMESTAMP |
| update_time | datetime | - | 否 | 否 | 否 | 最后修改时间 | ON UPDATE CURRENT_TIMESTAMP |
| create_user | bigint | - | 否 | 否 | 否 | 创建人 ID | 关联 employee.id |
...
| dish_id | bigint | - | 否 | 否 | 否 | 菜品 ID | 关联 dish.id（删除菜品时级联删除） |
...
| category_id | bigint | - | 否 | 否 | 否 | 分类 ID | 关联 category.id（type=2） |
...
| status | tinyint | - | 否 | 否 | 否 | 售卖状态 | 1 - 启售 0 - 停售（默认 1）；关联菜品停售时自动停售 |
| branch_id | bigint | - | 否 | 否 | 否 | 分店 ID | 关联 branch.id，支持分店独立套餐 |
| is_deleted | tinyint | - | 否 | 否 | 否 | 软删除标识 | 1 - 已删 0 - 未删（默认 0） |
| create_time | datetime | - | 否 | 否 | 否 | 创建时间 | 默认 CURRENT_TIMESTAMP |
| update_time | datetime | - | 否 | 否 | 否 | 最后修改时间 | ON UPDATE CURRENT_TIMESTAMP |
| create_user | bigint | - | 否 | 否 | 否 | 创建人 ID | 关联 employee.id |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| id | bigint | - | 否 | 是 | 是 | 关联 ID | 唯一标识 |
| setmeal_id | bigint | - | 否 | 否 | 否 | 套餐 ID | 关联 setmeal.id（删除套餐时级联删除） |
| dish_id | bigint | - | 否 | 否 | 否 | 菜品 ID | 关联 dish.id（菜品删除时级联删除） |
...
| branch_id | bigint | - | 否 | 否 | 否 | 分店 ID | 关联 branch.id |
...
| create_time | datetime | - | 否 | 否 | 否 | 创建时间 | 默认 CURRENT_TIMESTAMP |
| update_time | datetime | - | 否 | 否 | 否 | 最后修改时间 | ON UPDATE CURRENT_TIMESTAMP |
| update_user | bigint | - | 否 | 否 | 否 | 最后修改人 ID | 关联 employee.id（员工操作订单时记录） |
...
| order_id | bigint | - | 否 | 否 | 否 | 订单 ID | 关联 orders.id（删除订单时级联删除） |
| name | varchar | 32 | 否 | 否 | 否 | 商品名称 | 菜品 / 套餐名称 |
| image | varchar | 255 | 是 | 否 | 否 | 商品图片地址 | OSS 存储路径（冗余） |
| dish_id | bigint | - | 是 | 否 | 否 | 菜品 ID | 商品为菜品时非空，关联 dish.id |
| setmeal_id | bigint | - | 是 | 否 | 否 | 套餐 ID | 商品为套餐时非空，关联 setmeal.id |
...
| operator_id | bigint | - | 否 | 否 | 否 | 操作人 ID | 关联 employee.id |
| operator_name | varchar | 32 | 否 | 否 | 否 | 操作人姓名 | 冗余字段（避免关联员工表） |
| branch_id | bigint | - | 否 | 否 | 否 | 操作分店 ID | 关联 branch.id（数据范围审计） |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| id | bigint | - | 否 | 是 | 是 | 日志 ID | 唯一标识 |
| employee_id | bigint | - | 否 | 否 | 否 | 员工 ID | 关联 employee.id（未登录成功时为 0） |
| username | varchar | 32 | 否 | 否 | 否 | 登录用户名 | 即使登录失败也记录 |
| branch_id | bigint | - | 是 | 否 | 否 | 所属分店 ID | 登录成功后关联 employee.branch_id |
...
| dict_label | varchar | 32 | 否 | 否 | 否 | 字典标签 | 如 “待付款”“启售” |
...
| 批量上下架 | 批量修改菜品售卖状态（启售 / 停售） | 1. 批量操作需记录操作日志；2. 仅管理员 / 分店管理员可操作 |
...
| 状态联动 | 关联菜品停售时，套餐自动停售并记录日志 | 1. 联动停售后需手动重新启售；2. 记录操作日志（系统自动操作） |
| 批量上下架 | 批量修改套餐售卖状态（启售 / 停售） | 1. 批量操作需记录操作日志；2. 仅管理员 / 分店管理员可操作 |
...
*   **日期参数**：所有时间范围查询参数格式为 `yyyy-MM-dd HH:mm:ss`（如 `beginTime=2025-01-01 00:00:00`）
*   **操作人 / 操作时间**：所有增删改接口自动记录 `create_user`/`update_user`/`create_time`/`update_time`，无需前端传入
*   **JWT Payload 需包含**：`empId` (员工ID), `branchId` (分店ID), `username` (用户名)。不包含敏感信息（如密码）。
*   **配置说明**：在 `application.yml` 中预留阿里云 OSS 的配置项：`aliyun.oss.endpoint`, `aliyun.oss.accessKeyId`, `aliyun.oss.bucketName`。并且我的环境变量里已经有 `OSS_ACCESS_KEY_ID`, `OSS_ACCESS_KEY_SECRET`。
*   **版本说明**：
    *   JDK 版本为 21.0.8
    *   Node.js 版本为 v24.10.0

### 5.1 登录认证模块 (Auth)

| 接口名称 | 请求方式 | 路径 | 请求参数 (JSON Body) | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 员工登录 | POST | `/admin/auth/login` | `{username, password}` | `{token, id, username, name, roleId, branchId}` | 登录成功返回 JWT Token |
| 员工退出 | POST | `/admin/auth/logout` | - | - | 清除 Token |
| 账号解锁 | POST | `/admin/auth/unlock` | `{employeeId}` | - | 管理员解锁员工账号 |
| 强制解锁 | POST | `/admin/auth/force-unlock/{username}` | - | - | 临时接口，按用户名解锁 |

### 5.2 员工管理模块 (Employee)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 分页查询 | GET | `/admin/employee/list` | `?pageNum=1&pageSize=10&name=xx` | `PageInfo<EmployeeVO>` | 支持按姓名、手机号、状态查询 |
| 新增员工 | POST | `/admin/employee/save` | `{username, name, phone, sex, idNumber, roleId, branchId}` | `EmployeeVO` | 初始密码 123456 |
| 修改员工 | PUT | `/admin/employee/update` | `{id, name, phone, roleId, branchId, status}` | - | - |
| 密码重置 | POST | `/admin/employee/resetPwd` | `{employeeId}` | - | 重置为 123456 |
| 批量操作 | POST | `/admin/employee/batch` | `{ids: [1,2], operation: "lock/unlock/delete"}` | - | 批量锁定/启用/删除 |

### 5.3 角色管理模块 (Role)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 分页查询 | GET | `/admin/role/list` | `?pageNum=1&pageSize=10` | `PageInfo<RoleVO>` | - |
| 新增角色 | POST | `/admin/role/save` | `{name, description, status, permissionIds}` | `RoleVO` | - |
| 修改角色 | PUT | `/admin/role/update` | `{id, name, description, status, permissionIds}` | - | - |
| 删除角色 | DELETE | `/admin/role/delete/{id}` | - | - | - |

### 5.4 权限管理模块 (Permission)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 获取权限树 | GET | `/admin/permission/tree` | - | `List<PermissionVO>` | 返回树形结构权限数据 |

### 5.5 分店管理模块 (Branch)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 分页查询 | GET | `/admin/branch/list` | `?pageNum=1&pageSize=10` | `PageInfo<BranchVO>` | - |
| 新增分店 | POST | `/admin/branch/save` | `{name, address, contactName, contactPhone}` | `BranchVO` | - |
| 修改分店 | PUT | `/admin/branch/update` | `{id, name, address, contactName, contactPhone, status}` | - | - |
| 删除分店 | DELETE | `/admin/branch/delete/{id}` | - | - | - |

### 5.6 分类管理模块 (Category)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 分页查询 | GET | `/admin/category/list` | `?pageNum=1&pageSize=10&type=1` | `PageInfo<CategoryVO>` | type: 1-菜品 2-套餐 |
| 新增分类 | POST | `/admin/category/save` | `{name, type, sort, branchId}` | `CategoryVO` | - |
| 修改分类 | PUT | `/admin/category/update` | `{id, name, sort, status}` | - | - |
| 删除分类 | DELETE | `/admin/category/delete/{id}` | - | - | - |
| 批量操作 | POST | `/admin/category/batch` | `{ids: [1,2], operation: "enable/disable"}` | - | 批量启用/禁用 |
| 拖拽排序 | POST | `/admin/category/sort` | `{sortList: [{id:1, sort:0}, {id:2, sort:1}]}` | - | - |

### 5.7 菜品管理模块 (Dish)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 分页查询 | GET | `/admin/dish/list` | `?pageNum=1&pageSize=10&categoryId=1` | `PageInfo<DishVO>` | - |
| 新增菜品 | POST | `/admin/dish/save` | `{name, categoryId, price, image, description, flavors, specifications}` | `DishVO` | - |
| 修改菜品 | PUT | `/admin/dish/update` | `{id, name, categoryId, price, image, ...}` | - | - |
| 删除菜品 | DELETE | `/admin/dish/delete/{id}` | - | - | 软删除（移入回收站） |
| 批量操作 | POST | `/admin/dish/batch` | `{ids: [1,2], operation: "enable/disable"}` | - | 批量启售/停售 |
| 导入菜品 | POST | `/admin/dish/import` | `multipart/form-data: file` | - | Excel 导入 |
| 导出菜品 | GET | `/admin/dish/export` | `?categoryId=1` | - | Excel 导出（下载） |
| 恢复菜品 | PUT | `/admin/dish/restore/{id}` | - | - | 从回收站恢复 |
| 永久删除 | DELETE | `/admin/dish/permanent/{id}` | - | - | 物理删除 |

### 5.8 套餐管理模块 (Setmeal)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 分页查询 | GET | `/admin/setmeal/list` | `?pageNum=1&pageSize=10&categoryId=2` | `PageInfo<SetmealVO>` | - |
| 新增套餐 | POST | `/admin/setmeal/save` | `{name, categoryId, price, image, description, setmealDishes}` | `SetmealVO` | - |
| 修改套餐 | PUT | `/admin/setmeal/update` | `{id, name, categoryId, price, image, ...}` | - | - |
| 删除套餐 | DELETE | `/admin/setmeal/delete/{id}` | - | - | 软删除（移入回收站） |
| 批量操作 | POST | `/admin/setmeal/batch` | `{ids: [1,2], operation: "enable/disable"}` | - | 批量启售/停售 |
| 导出套餐 | GET | `/admin/setmeal/export` | `?categoryId=2` | - | Excel 导出（下载） |
| 恢复套餐 | PUT | `/admin/setmeal/restore/{id}` | - | - | 从回收站恢复 |
| 永久删除 | DELETE | `/admin/setmeal/permanent/{id}` | - | - | 物理删除 |

### 5.9 订单管理模块 (Order)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 分页查询 | GET | `/admin/order/list` | `?pageNum=1&pageSize=10&status=1` | `PageInfo<OrderVO>` | - |
| 订单详情 | GET | `/admin/order/detail` | `?id=1` | `OrderVO` | 含订单明细 |
| 修改状态 | PUT | `/admin/order/updateStatus` | `{id, status}` | - | 接单/派送/完成 |
| 取消订单 | POST | `/admin/order/cancel` | `{id, cancelReason}` | - | 取消/拒单 |
| 导出订单 | GET | `/admin/order/export` | `?status=1` | - | Excel 导出（下载） |

### 5.10 数据统计模块 (Stat)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 营业统计 | GET | `/admin/stat/business` | `?beginTime=xx&endTime=xx` | `StatBusinessVO` | 营业额/订单数等 |
| 销量排行 | GET | `/admin/stat/salesTop` | `?beginTime=xx&endTime=xx` | `List<StatSalesTopVO>` | TOP10 销量 |
| 订单分析 | GET | `/admin/stat/orderAnalysis` | `?beginTime=xx&endTime=xx` | `Object` | 订单状态分布 |

### 5.11 日志管理模块 (Log)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 登录日志 | GET | `/admin/log/loginList` | `?pageNum=1&pageSize=10` | `PageInfo<?>` | - |

### 5.12 数据字典模块 (Dict)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 字典列表 | GET | `/admin/dict/list` | `?dictType=order_status` | `List<DictVO>` | - |
| 新增字典 | POST | `/admin/dict/save` | `{dictType, dictCode, dictLabel}` | `DictVO` | - |

### 5.13 通用接口 (Common)

| 接口名称 | 请求方式 | 路径 | 请求参数 | 返回数据 (Data) | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 上传图片 | POST | `/admin/common/upload` | `multipart/form-data: file` | `{url: "http..."}` | 上传至 OSS |
| 图片代理 | GET | `/admin/common/image-proxy` | `?url=http...` | - | 解决跨域/ORB问题 |
...
│   ├── common          # 通用模块 (异常, 结果封装, 工具类)
│   ├── config          # 配置类 (WebMvc, Mybatis, Swagger等)
│   ├── controller      # 控制层 (Admin接口)
│   ├── dto             # 数据传输对象 (接收前端参数)
│   ├── entity          # 实体类 (数据库映射)
│   ├── mapper          # 持久层接口
│   ├── service         # 业务逻辑接口
...
### 7.2 关键参数
- **Database**: MySQL 8.0+ (`deliver_management`)
- **Object Storage**: Aliyun OSS (Requires `OSS_ACCESS_KEY_ID`, `OSS_ACCESS_KEY_SECRET`)
- **Security**: JWT (Secret key configured in `delivery.jwt.secret-key`)
...
### 8.2 启动步骤
1. 创建数据库 `deliver_management` 并导入 `init.sql`。
2. 配置环境变量 `OSS_ACCESS_KEY_ID` 和 `OSS_ACCESS_KEY_SECRET`。
3. 修改 `application-dev.yml` 中的数据库账号密码。
4. 运行 `mvn spring-boot:run` 或在 IDE 中启动 `DeliverManagementApplication`。

### 8.3 前端运行
1. 进入前端目录 (假设为 `deliver-management-web`)。
